...
pub struct MultiTokenTokenizer<'s, const MAX_NON_EMTY_TOKENS: usize> {
    tokenizer: SimpleTokenizer<'s>,
    multi_token: MultiToken<'s>,
    is_empty_token_predicate: &'s dyn Fn(&SimpleToken<'s>) -> bool,
    cur_non_empty: usize,
    is_ended: bool,
}

impl<'s, const MAX_NON_EMTY_TOKENS: usize> MultiTokenTokenizer<'s, MAX_NON_EMTY_TOKENS> {
    pub fn new(token_stream: &'s str, is_empty_token_predicate: &'s dyn Fn(&SimpleToken<'s>) -> bool) -> Self {
        Self {
